...
const QUALITY_INFO: Record<string, { label: string; desc: string; features: string[] }> = {
  ...
  standard: {
    ...
    features: ["PPC/OPC 53 cement, Fe500D steel", "Fly ash brick masonry", "GVT/PGVT tiles 600×600", "Premium emulsion paint (2 coats)", "Mid-range sanitary (Parryware/Cera)"],
  },
  premium: {
    label: "Premium",
...
export const ConstructionCalculator = () => {
  ...
  const handleDownloadPDF = () => {
    const rows: [string, string, string][] = [
      ["Construction",    `${area} sq.ft @ ₹${customRate}/sqft`,   formatCurrency(costs.main)],
    ];
              ...
              <div className="grid grid-cols-3 gap-3">
                {(["basic","standard","premium"] as const).map(q => (
                  <button key={q} type="button"
                    ...
                    <span className="text-xs opacity-70">₹{QUALITY_RATES[q].toLocaleString()}/sqft</span>
                  </button>
                ))}
              </div>
              {/* Quality Features */}
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-4">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                <div className="flex flex-col">
                  <span className="text-sm font-semibold text-gray-600">Custom Rate (₹/sq.ft)</span>
                  <button type="button" onClick={() => setIsEditingRate(!isEditingRate)}
                    className="text-xs text-primary hover:underline text-left">
                    {isEditingRate ? "← Reset to Default" : "Edit Rate ✏️"}
                  </button>
                </div>
                <div className="relative w-36">
                  <input type="number" value={customRate} onChange={e => { setCustomRate(parseFloat(e.target.value)); setIsEditingRate(true); }}
                  ...
                  <span className="absolute right-8 top-2 text-xs text-gray-400 pointer-events-none">₹/sqft</span>
                </div>
              </div>
            </div>
          </form>
        ...
        <Card title="Reference Information">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 text-xs text-gray-500 uppercase font-bold">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right">Basic (₹/sqft)</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {CITY_BENCHMARKS.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-right text-gray-600">₹{row.basic.toLocaleString()}</td>
                      <td className="px-3 py-2.5 text-right text-gray-600">₹{row.standard.toLocaleString()}</td>
                      <td className="px-3 py-2.5 text-right font-semibold text-gray-700">₹{row.premium.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <p className="text-xs text-gray-400 mt-3">* Indicative 2024–25 rates. Actual rates depend on specific site conditions, design complexity and local labour market.</p>
            </div>
          )}
          ...
          {activeTab === "timeline" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-400 mt-2">* Timeline for a G+1 structure (~1500 sqft). Larger projects scale proportionally.</p>
            </div>
          )}
        </Card>
      </section>
          ...
          <>
            ...
            <Card className="border-primary/30 shadow-float relative overflow-hidden bg-white">
              ...
              <div className="text-center py-4 relative z-10">
                ...
                <p className="mt-2 text-sm text-gray-400">≈ <span className="font-bold text-gray-600">{formatCurrency(perSqftCost)}/sq.ft</span> effective rate</p>
              </div>
              {/* Mini KPI Strip */}
              <div className="grid grid-cols-3 gap-2 mt-2">
                {[
                  ...
                  { label:"Effective Rate",    value: `₹${perSqftCost}/sqft`, color:"text-primary" },
                ].map((k,i) => (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    <tr className="bg-primary/5">
                      <td className="px-4 py-3 font-semibold text-secondary">Construction <span className="text-xs text-gray-400 block font-normal">({area} sqft @ ₹{customRate})</span></td>
                      <td className="px-4 py-3 text-right font-bold">{formatCurrency(costs.main)}</td>
                      <td className="px-4 py-3 text-right text-gray-500 hidden sm:table-cell">{totalCost > 0 ? Math.round((costs.main/totalCost)*100) : 0}%</td>
                    </tr>
                    {costs.parking > 0 && (
                      <tr><td className="px-4 py-3 text-gray-700">Parking ({parkingArea} sqft)</td>
                        <td className="px-4 py-3 text-right text-gray-600">{formatCurrency(costs.parking)}</td>
                        <td className="px-4 py-3 text-right text-gray-400 hidden sm:table-cell">{Math.round((costs.parking/totalCost)*100)}%</td></tr>
                    )}
                    {costs.wall > 0 && (
                      <tr><td className="px-4 py-3 text-gray-700">Compound Wall ({compoundWallLength} ft)</td>
                        <td className="px-4 py-3 text-right text-gray-600">{formatCurrency(costs.wall)}</td>
                        <td className="px-4 py-3 text-right text-gray-400 hidden sm:table-cell">{Math.round((costs.wall/totalCost)*100)}%</td></tr>
                    )}
                    {costs.sump > 0 && (
                      <tr><td className="px-4 py-3 text-gray-700">Sump & Septic</td>
                        <td className="px-4 py-3 text-right text-gray-600">{formatCurrency(costs.sump)}</td>
                        <td className="px-4 py-3 text-right text-gray-400 hidden sm:table-cell">{Math.round((costs.sump/totalCost)*100)}%</td></tr>
                    )}
                  </tbody>
                </table>
              </div>

              {/* Structural Breakdown sub-items */}
              <div className="mb-4">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Structural Cost Split (of construction amount)</p>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
              ...
              <strong>Note:</strong> This estimate covers structural and finishing costs only. Land cost, architect fees, government approvals, utility connections, and furniture are not included. Add 10–15% contingency buffer to your budget.
            </div>
