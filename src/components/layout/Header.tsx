      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            {user ? (
              <>
                ...
                <button onClick={handleLogout} className="px-4 py-2 text-sm font-medium text-red-500 border border-red-200 rounded-full hover:bg-red-50 transition-all">
                  Sign Out
                </button>
              </>
            ) : (
          ...
          <div className="md:hidden flex items-center">
            <button onClick={() => setMenuOpen(!menuOpen)} className="text-gray-600 hover:text-primary focus:outline-none p-2">
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 absolute w-full left-0 shadow-lg">
          <div className="px-4 pt-2 pb-4 space-y-2 flex flex-col">
            ...
            {user ? (
              <>
                <Link to="/dashboard" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-primary hover:bg-gray-50" onClick={() => setMenuOpen(false)}>
                  Dashboard
                </Link>
              ...
              <Link to="/signin" className="block px-3 py-2 mt-4 text-center rounded-md text-base font-bold text-white bg-primary" onClick={() => setMenuOpen(false)}>
                Sign In
              </Link>
            )}
          </div>
