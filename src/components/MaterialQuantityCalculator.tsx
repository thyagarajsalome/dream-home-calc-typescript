...
const wallMaterialTypes = {
  redBrick: { name: "Red Bricks", cementFactor: 1.0, sandFactor: 1.0, unit: "Nos", countPerSqFt: 12 },
  flyAsh: { name: "Fly Ash Bricks", cementFactor: 0.9, sandFactor: 0.9, unit: "Nos", countPerSqFt: 11 },
  aac: { name: "AAC Blocks", cementFactor: 0.5, sandFactor: 0.4, unit: "Nos", countPerSqFt: 8 },
};
...
const defaultPrices = {
  cement: 400, steel: 65, sand: 60, aggregate: 40, paint: 350, tiles: 600, bricks: 10,
};
...
const MaterialQuantityCalculator: React.FC = () => {
  ...
  const [wallType, setWallType] = useState<keyof typeof wallMaterialTypes>("redBrick");
  ...
  const calculateQuantities = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const cement = parsedArea * qRule.cement * cRule.cementMultiplier * ((wRule.cementFactor + 1) / 2);
    ...
    const totalMaterialCost = costCement + costSteel + costSand + costAggregate + costWall + costPaint + costTiles;
    
    setResults({
      cement, sand, aggregate, steel,
      wallMaterialName: wRule.name, wallMaterialCount: wallCount,
      paint: paintLiters, tiles: tileBoxes,
      costCement, costSteel, costSand, costAggregate, costWall, costPaint, costTiles,
      totalMaterialCost,
    });
  };
      ...
      <div className="card">
        ...
        <form onSubmit={calculateQuantities}>
          <div className="form-group">
            ...
            <input type="number" value={area} onChange={(e) => setArea(e.target.value)} disabled={!hasPaid} placeholder="e.g. 1000" />
          </div>
          <div className="form-grid">
            <div className="form-group">
              <label>Wall Material</label>
              <select value={wallType} onChange={(e) => setWallType(e.target.value as any)} disabled={!hasPaid}>
            ...
            <div className="form-group">
              ...
              <select value={grade} onChange={(e) => setGrade(e.target.value as any)} disabled={!hasPaid}>
            ...
            <div className="card" ref={resultsRef}>
              ...
              <div className="results-grid" style={{ gridTemplateColumns: "1fr" }}>
                <div className="result-details">
                  ...
                  <table>
                    <thead><tr><th>Material</th><th>Quantity</th><th className="text-right">Approx Cost</th></tr></thead>
                    <tbody>
                      <tr><td>Cement</td><td>{formatNumber(results.cement)} Bags</td><td className="text-right">{formatCurrency(results.costCement)}</td></tr>
                      <tr><td>Steel</td><td>{formatNumber(results.steel)} kg</td><td className="text-right">{formatCurrency(results.costSteel)}</td></tr>
                      <tr><td>Sand</td><td>{formatNumber(results.sand)} cft</td><td className="text-right">{formatCurrency(results.costSand)}</td></tr>
                      <tr><td>Aggregate</td><td>{formatNumber(results.aggregate)} cft</td><td className="text-right">{formatCurrency(results.costAggregate)}</td></tr>
                      <tr><td>{results.wallMaterialName}</td><td>{formatNumber(results.wallMaterialCount)} {wallMaterialTypes[wallType as keyof typeof wallMaterialTypes].unit}</td><td className="text-right">{formatCurrency(results.costWall)}</td></tr>
                      <tr><td>Paint</td><td>{formatNumber(results.paint)} L</td><td className="text-right">{formatCurrency(results.costPaint)}</td></tr>
                      <tr><td>Tiles</td><td>{formatNumber(results.tiles)} Boxes</td><td className="text-right">{formatCurrency(results.costTiles)}</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
              {hasPaid && (
                <div className="action-buttons">
                  <button className="btn" onClick={downloadPDF} disabled={isDownloading}><i className="fas fa-download"></i> {isDownloading ? "Processing..." : "Download PDF"}</button>
